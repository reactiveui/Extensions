...
public static partial class ObservableAsync
{
    ...
    /// <summary>
    /// Async observable that combines the latest values from six source sequences using a selector.
    /// </summary>
        ...
        Func<T1, T2, T3, T4, T5, T6, TResult> selector) : ObservableAsync<TResult>
    {
        ...
        internal sealed class CombineLatestSubscription : IAsyncDisposable
        {
            ...
            private readonly IAsyncDisposable?[] _subscriptions = new IAsyncDisposable?[6];
            ...
            private async ValueTask EmitLatestAsync(CancellationToken cancellationToken)
            {
                ...
                using (await _gate.LockAsync(_disposeCancellationToken).ConfigureAwait(false))
                {
                    ...
                    await _observer.OnNextAsync(_selector(values.V1, values.V2, values.V3, values.V4, values.V5, values.V6), _disposeCancellationToken).ConfigureAwait(false);
                }
            }
        }
    }
