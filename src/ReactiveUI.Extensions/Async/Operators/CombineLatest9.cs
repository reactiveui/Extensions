...
/// <summary>
/// Provides the arity-9 (<c>nine</c>-source) <c>CombineLatest</c> extension method
...
{
    /// <summary>
    /// Combines the latest values from nine asynchronous observable sources into a single
    ...
    public static IObservableAsync<TResult> CombineLatest<T1, T2, T3, T4, T5, T6, T7, T8, T9, TResult>(
        ...
        new CombineLatest9ObservableAsync<T1, T2, T3, T4, T5, T6, T7, T8, T9, TResult>(
            new(src1, src2, src3, src4, src5, src6, src7, src8, src9),
            selector);

    /// <summary>
    /// Async observable that combines the latest values from nine source sequences using a selector.
    /// </summary>
    ...
    /// <typeparam name="TResult">The projected element type.</typeparam>
    internal sealed class CombineLatest9ObservableAsync<T1, T2, T3, T4, T5, T6, T7, T8, T9, TResult>(
        CombineLatest9ObservableAsync<T1, T2, T3, T4, T5, T6, T7, T8, T9, TResult>.Sources sources,
        Func<T1, T2, T3, T4, T5, T6, T7, T8, T9, TResult> selector) : ObservableAsync<TResult>
    {
        /// <summary>
        /// Bundles the nine source observables so the subscription constructor stays at three
        ...
        internal sealed class CombineLatestSubscription : IAsyncDisposable
        {
            ...
            private readonly IAsyncDisposable?[] _subscriptions = new IAsyncDisposable?[9];
            ...
            private bool TryReadValues(out Values values)
            {
                ...
                {
                    values = new(v1, v2, v3, v4, v5, v6, v7, v8, v9);
                    return true;
                }
            ...
            private async ValueTask EmitLatestAsync(CancellationToken cancellationToken)
            {
                ...
                using (await _gate.LockAsync(_disposeCancellationToken).ConfigureAwait(false))
                {
                    ...
                    await _observer.OnNextAsync(
                            _selector(values.V1, values.V2, values.V3, values.V4, values.V5, values.V6, values.V7, values.V8, values.V9),
