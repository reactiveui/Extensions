        ...
        Func<T1, T2, T3, T4, T5, T6, T7, TResult> selector) : ObservableAsync<TResult>
    {
        ...
        internal sealed class CombineLatestSubscription : IAsyncDisposable
        {
                ...
                Func<T1, T2, T3, T4, T5, T6, T7, TResult> selector)
            {
                _sources = sources;
                _selector = selector;
                Lifecycle = new CombineLatestLifecycle<TResult>(observer, sourceCount: 7);
            }
            ...
            private ValueTask EmitLatestAsync() =>
                TryReadValues(out var values)
                    ? Lifecycle.EmitDownstreamAsync(_selector(values.V1, values.V2, values.V3, values.V4, values.V5, values.V6, values.V7))
                    : default;
        }
    }
}
