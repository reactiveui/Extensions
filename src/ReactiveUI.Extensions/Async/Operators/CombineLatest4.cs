...
public static partial class ObservableAsync
{
        ...
        Func<T1, T2, T3, T4, TResult> selector) : ObservableAsync<TResult>
    {
        ...
        internal sealed class CombineLatestSubscription : IAsyncDisposable
        {
            ...
            private readonly IAsyncDisposable?[] _subscriptions = new IAsyncDisposable?[4];
            ...
            private async ValueTask EmitLatestAsync(CancellationToken cancellationToken)
            {
                ...
                using (await _gate.LockAsync(_disposeCancellationToken).ConfigureAwait(false))
                {
                    ...
                    await _observer.OnNextAsync(_selector(values.V1, values.V2, values.V3, values.V4), _disposeCancellationToken).ConfigureAwait(false);
                }
            }
        }
    }
