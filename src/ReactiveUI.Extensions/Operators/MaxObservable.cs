...
/// <summary>
/// Optimized operator that combines the latest values from multiple sources and emits the maximum.
/// Replaces .CombineLatest().Select(x => x.Max()) to avoid array allocations on every emission.
/// </summary>
/// <typeparam name="T">The value type.</typeparam>
/// <param name="sources">The source observables.</param>
internal sealed class MaxObservable<T>(IEnumerable<IObservable<T>> sources) : IObservable<T>
    where T : struct, IComparable<T>
{
    ...
    public IDisposable Subscribe(IObserver<T> observer)
    {
        ...
        var sink = new MaxSink(observer, _sourceList.Count);
    ...
    /// <summary>
    /// Sink that manages the combined state for <see cref="MaxObservable{T}"/>.
    /// </summary>
    /// <param name="downstream">The downstream observer.</param>
    /// <param name="count">The number of sources.</param>
    private sealed class MaxSink(IObserver<T> downstream, int count)
    {
