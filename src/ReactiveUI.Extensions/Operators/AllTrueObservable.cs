...
/// <summary>
/// Optimized operator that combines latest booleans and emits true only when all are true.
...
internal sealed class AllTrueObservable(IEnumerable<IObservable<bool>> sources) : IObservable<bool>
{
    ...
    public IDisposable Subscribe(IObserver<bool> observer)
    {
        ...
        var sink = new AllTrueSink(observer, _sourceList.Count);
    ...
    /// <summary>
    /// Sink that manages the combined state for <see cref="AllTrueObservable"/>.
    /// </summary>
    /// <param name="downstream">The downstream observer.</param>
    /// <param name="count">The number of sources.</param>
    private sealed class AllTrueSink(IObserver<bool> downstream, int count)
    {
