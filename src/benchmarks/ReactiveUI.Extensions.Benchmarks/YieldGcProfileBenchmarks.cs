...
/// <summary>GC-verbose hotspot profile for <c>Yield</c>'s per-emission threadpool hop.</summary>
[ShortRunJob]
[MemoryDiagnoser]
[EventPipeProfiler(EventPipeProfile.GcVerbose)]
public class YieldGcProfileBenchmarks : IDisposable
{
    /// <summary>Number of emissions pushed through the pipeline per invocation.</summary>
    private const int EmissionCount = 1_000;
    ...
    /// <summary>Source feeding the Yield pipeline.</summary>
    private SerialStatelessSubjectAsync<int> _source = null!;

    /// <summary>Subscription on the Yield pipeline.</summary>
    private IAsyncDisposable _subscription = null!;

    /// <summary>Wires the Yield pipeline.</summary>
    /// <returns>A task that completes when the pipeline is subscribed.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _source = new SerialStatelessSubjectAsync<int>();
        _subscription = await _source.Yield().SubscribeAsync(_sink, default).ConfigureAwait(false);
    }
    ...
    /// <summary>Drives <see cref="EmissionCount"/> values through the Yield pipeline.</summary>
    /// <returns>A task that completes when every value has been propagated.</returns>
    [Benchmark]
    public async Task Yield_PerEmission()
    {
