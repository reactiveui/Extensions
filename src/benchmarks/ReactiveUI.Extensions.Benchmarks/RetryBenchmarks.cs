...
/// <summary>
/// Measures the per-emission cost of <c>Retry</c> on the happy path. The upstream never errors,
...
public class RetryBenchmarks : IDisposable
{
    ...
    /// <summary>Source feeding the Retry pipeline.</summary>
    private SerialStatelessSubjectAsync<int> _source = null!;

    /// <summary>Subscription on the Retry pipeline.</summary>
    ...
    /// <summary>Drives <see cref="EmissionCount"/> values through the Retry pipeline (no error path).</summary>
    /// <returns>A task that completes when every value has been propagated.</returns>
    [Benchmark]
    public async Task Retry_HappyPath()
    {
