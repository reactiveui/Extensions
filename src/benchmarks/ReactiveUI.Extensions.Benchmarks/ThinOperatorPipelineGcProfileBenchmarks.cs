...
/// <summary>
/// GC-verbose hotspot profile for the thin operator pipeline (Select → Where → Scan composed over
/// <see cref="ObserverAsync{T}"/>). Pairs with <see cref="ThinOperatorBenchmarks"/> for steady-state numbers;
/// this class adds <c>[EventPipeProfiler(EventPipeProfile.GcVerbose)]</c> so each run produces a
/// <c>.nettrace</c> consumable by <c>rxext-allocreport</c> for per-type and per-stack allocation tables.
/// </summary>
[ShortRunJob]
[MemoryDiagnoser]
[EventPipeProfiler(EventPipeProfile.GcVerbose)]
public class ThinOperatorPipelineGcProfileBenchmarks : IDisposable
{
