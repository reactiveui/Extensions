...
/// <summary>
/// Measures the per-emission steady-state cost of <c>CatchAndIgnoreErrorResume</c> on the
/// happy path — every emission forwards verbatim, and the fallback factory is never invoked.
/// </summary>
[SimpleJob(RuntimeMoniker.Net10_0)]
[MemoryDiagnoser]
[MarkdownExporterAttribute.GitHub]
public class CatchAndIgnoreErrorResumeBenchmarks : IDisposable
{
    ...
    /// <summary>Pre-built fallback observable; never subscribed on the happy path.</summary>
    private readonly IObservableAsync<int> _fallback = ObservableAsync.Return(-1);

    /// <summary>Source feeding the CatchAndIgnoreErrorResume pipeline.</summary>
    private SerialStatelessSubjectAsync<int> _source = null!;

    /// <summary>Subscription on the CatchAndIgnoreErrorResume pipeline.</summary>
    ...
    /// <summary>Wires the pipeline with a non-capturing fallback factory.</summary>
    /// <returns>A task that completes when the pipeline is subscribed.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _source = new SerialStatelessSubjectAsync<int>();
        _subscription = await _source
            .CatchAndIgnoreErrorResume(_ => _fallback)
    ...
    /// <summary>Drives <see cref="EmissionCount"/> values through the CatchAndIgnoreErrorResume pipeline (no errors).</summary>
    /// <returns>A task that completes when every value has been propagated.</returns>
    [Benchmark]
    public async Task CatchAndIgnoreErrorResume_HappyPath()
    {
