...
/// <summary>
/// Measures the per-emission cost of <c>Throttle</c> using a deterministic
/// <see cref="BenchmarkImmediateFireTimeProvider"/>; every emission's debounce timer fires
/// synchronously inside <c>CreateTimer</c>, so the steady-state numbers reflect the operator's
...
public class ThrottleBenchmarks : IDisposable
{
    ...
    /// <summary>Debounce window; ignored at the wall-clock level because the time provider fires synchronously.</summary>
    ...
    /// <summary>Source feeding the Throttle pipeline.</summary>
    private SerialStatelessSubjectAsync<int> _source = null!;

    /// <summary>Subscription on the Throttle pipeline.</summary>
    ...
    /// <summary>Drives <see cref="EmissionCount"/> values through the Throttle pipeline.</summary>
    /// <returns>A task that completes when every value has been propagated.</returns>
    [Benchmark]
    public async Task Throttle_ImmediateFire()
    {
