...
/// <summary>
/// Measures the per-emission cost of <c>GroupBy</c> across a small, fixed key set. New keys are
...
public class GroupByBenchmarks : IDisposable
{
    ...
    /// <summary>Number of distinct keys cycled through; chosen so all groups are pre-created and warm before measurement.</summary>
    private const int GroupCount = 4;

    /// <summary>Shared no-op sink for the outer GroupBy emission stream.</summary>
    private readonly BenchmarkNoopObserver<GroupedAsyncObservable<int, int>> _outerSink = new();

    /// <summary>Shared no-op sink wired into each inner group's emission stream.</summary>
    ...
    /// <summary>Wires the GroupBy pipeline and primes each group so the benchmark body hits only the steady-state lookup path.</summary>
    ...
    public async Task SetupAsync()
    {
        ...
        // Prime every group key so each future emission lands in an existing group.
