...
/// <summary>
/// Measures the per-emission cost of <c>DropIfBusy</c> when the async action completes synchronously
...
public class DropIfBusyBenchmarks : IDisposable
{
    ...
    /// <summary>Source feeding the <c>DropIfBusy</c> pipeline.</summary>
    ...
    /// <summary>Wires the pipeline against an async action that always completes synchronously.</summary>
    /// <returns>A task that completes when the pipeline is subscribed.</returns>
    [GlobalSetup]
    public async Task SetupAsync()
    {
        _source = new SerialStatelessSubjectAsync<int>();
        _subscription = await _source.DropIfBusy(static (_, _) => default)
    ...
    /// <summary>Drives values through <c>DropIfBusy</c>; each emission completes the inner action synchronously and is forwarded.</summary>
    /// <returns>A task that completes when every value has been propagated.</returns>
    [Benchmark]
    public async Task DropIfBusy_AllPassThrough()
    {
