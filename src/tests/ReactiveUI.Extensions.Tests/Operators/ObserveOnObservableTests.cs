...
public class ObserveOnObservableTests
{
    ...
    public async Task WhenImmediateScheduler_ThenForwardsDirectly()
    {
        ...
        using var sub = source.ObserveOnSafe(ImmediateScheduler.Instance)
    ...
    public async Task WhenSourceErrors_ThenForwardsError()
    {
        ...
        using var sub = source.ObserveOnSafe(scheduler).Subscribe(static _ => { }, ex => caught = ex);
    ...
    public async Task WhenSourceCompletes_ThenForwardsCompletion()
    {
        ...
        using var sub = source.ObserveOnSafe(scheduler).Subscribe(static _ => { }, () => completed = true);
    ...
    public async Task WhenSourceTerminatesDuringSubscribe_ThenLateAttachDisposesSubscription()
    {
        ...
        using var sub = source.ObserveOnSafe(new InlineScheduler())
    ...
    /// executes inline during the schedule call. Distinct instance from <see cref="ImmediateScheduler"/>
    /// so the operator's immediate-scheduler passthrough does not apply.</summary>
    private sealed class InlineScheduler : IScheduler
    {
        /// <inheritdoc/>
