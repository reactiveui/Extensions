...
/// <summary>Smoke tests for arity-11 CombineLatest covering the subscription lifecycle,
/// dispose guard, error forwarding, and the all-sources-emit happy path.</summary>
public partial class CombineLatestArityTests
{
    /// <summary>
    /// Verifies that CombineLatest11 disposes on subscription failure (catch block).
    /// </summary>
    ...
    public async Task WhenCombineLatest11SubscriptionThrows_ThenDisposesAndRethrows()
    {
    ...
    /// <summary>
    /// Verifies that CombineLatest11 OnNextCombined guard returns when disposed.
    /// </summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous test operation.</returns>
    [Test]
    [SuppressMessage("Major Code Smell", "S107:Methods should not have too many parameters", Justification = "Test Reasons")]
    [SuppressMessage(
        ...
        "S138:Methods should not have too many lines",
        Justification = "Smoke test inherently lists N subjects + per-source calls; splitting would obscure the under-test sequence.")]
    public async Task WhenCombineLatest11DisposedBeforeCombine_ThenOnNextCombinedIsGuarded()
    {
        var s1 = SubjectAsync.Create<int>();
        var s2 = SubjectAsync.Create<int>();
        var s3 = SubjectAsync.Create<int>();
        ...
        var sub = await s1.Values
            .CombineLatest(
                ...
                s10.Values,
                s11.Values,
                (v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11) => v1 + v2 + v3 + v4 + v5 + v6 + v7 + v8 + v9 + v10 + v11).SubscribeAsync(
                (x, _) =>
                {
                    results.Add(x);
                    return default;
    ...
    /// <summary>
    /// Verifies that CombineLatest11 forwards a source error to the downstream observer.
    /// </summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous test operation.</returns>
    [Test]
    [SuppressMessage("Major Code Smell", "S107:Methods should not have too many parameters", Justification = "Test Reasons")]
    [SuppressMessage(
        ...
        "S138:Methods should not have too many lines",
        Justification = "Smoke test inherently lists N subjects + per-source calls; splitting would obscure the under-test sequence.")]
    public async Task WhenCombineLatest11OneSourceErrors_ThenCombinedErrorForwarded()
    {
        var s1 = SubjectAsync.Create<int>();
        var s2 = SubjectAsync.Create<int>();
        var s3 = SubjectAsync.Create<int>();
    ...
    /// <summary>
    /// Verifies that CombineLatest11 produces the selector's result once every source has emitted at least once.
    /// </summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous test operation.</returns>
    [Test]
    [SuppressMessage("Major Code Smell", "S107:Methods should not have too many parameters", Justification = "Test Reasons")]
    [SuppressMessage(
        ...
        "S138:Methods should not have too many lines",
        Justification = "Smoke test inherently lists N subjects + per-source calls; splitting would obscure the under-test sequence.")]
    public async Task WhenCombineLatest11AllSourcesEmit_ThenSelectorResultEmitted()
    {
        var s1 = SubjectAsync.Create<int>();
        var s2 = SubjectAsync.Create<int>();
        var s3 = SubjectAsync.Create<int>();
    ...
    /// <summary>
    /// Verifies that CombineLatest11 completes once every source has completed.
    /// </summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous test operation.</returns>
    [Test]
    [SuppressMessage("Major Code Smell", "S107:Methods should not have too many parameters", Justification = "Test Reasons")]
    [SuppressMessage(
        ...
        "S138:Methods should not have too many lines",
        Justification = "Smoke test inherently lists N subjects + per-source calls; splitting would obscure the under-test sequence.")]
    public async Task WhenCombineLatest11AllSourcesComplete_ThenCombinedCompletes()
    {
        var s1 = SubjectAsync.Create<int>();
        var s2 = SubjectAsync.Create<int>();
        var s3 = SubjectAsync.Create<int>();
